...
class SettingsFrame(wx.Frame):
    ...
    def _initLayout(self):
        ...
        lblInterval = wx.StaticText(
            panel, -1, 'Intervalo (dias)', size=(widthLabel, -1))
        self.txtInterval = wx.TextCtrl(panel, -1, '', size=(50, -1))

        lblDest = wx.StaticText(
        ...
        self.txtStartTime = wx.adv.TimePickerCtrl(panel, size=(120, -1),
                                                  style=wx.adv.TP_DEFAULT)

        lblTimeTo = wx.StaticText(
            panel, -1, 'até', size=(18, -1))

        self.txtEndTime = wx.adv.TimePickerCtrl(panel, size=(120, -1),
                                                style=wx.adv.TP_DEFAULT)
        ...
        timeSizer.Add(self.txtStartTime,
                      wx.SizerFlags(0).Border(wx.LEFT, 5))
        timeSizer.Add(lblTimeTo, wx.SizerFlags(0).Align(wx.ALIGN_CENTER_VERTICAL).Border(wx.LEFT | wx.RIGHT, 5))
        timeSizer.Add(self.txtEndTime)
        ...
        sourceSizer.Add(self.btnRemoveSource,
                        wx.SizerFlags(0).Border(wx.LEFT, 5))

        sourceBoxSizer = wx.BoxSizer(wx.VERTICAL)
        borderTop = 15 if sys.platform == 'win32' else 0
        sourceBoxSizer.Add(sourceSizer, wx.SizerFlags(
            0).Expand().Border(wx.TOP, borderTop))
        sourceBoxSizer.Add(self.listSources, wx.SizerFlags(
            0).Expand().Border(wx.TOP, 5))

        sourceBoxSizerOuter = wx.BoxSizer(wx.VERTICAL)
        sourceBoxSizerOuter.Add(sourceBoxSizer, wx.SizerFlags(
            0).Expand().Border(wx.ALL, 10))
    ...
    def showInvalidIntervalDialog(self):
        wx.MessageDialog(self, 'Selecione um intervalo válido',
                         'Intervalo inválido').ShowModal()

    def showInvalidDestDialog(self):
        wx.MessageDialog(self, 'Selecione uma pasta de destino válida',
                         'Pasta de destino inválida').ShowModal()

    def showInvalidSourceDialog(self):
        wx.MessageDialog(self, 'Selecione ao menos um arquivo para ser copiado',
                         'Arquivos inválidos').ShowModal()

    def showConfirmChangesErrorDialog(self):
        wx.MessageDialog(self, 'Não foi possível salvar os dados',
                         'Error inesperado').ShowModal()
...
class SettingsPresenter:
    ...
    def _loadViewFromModel(self):
        ...
        if self._scheduleModel.startTime:
            self._view.txtStartTime.SetTime(*self._scheduleModel.startTimeTuple)
        else:
            self._view.txtStartTime.SetTime(0, 0, 0)

        if self._scheduleModel.endTime:
            self._view.txtEndTime.SetTime(*self._scheduleModel.endTimeTuple)
        else:
            self._view.txtEndTime.SetTime(0, 0, 0)
        ...
        for source in self._copyModel.sources:
            self._view.listSources.InsertItem(
                self._view.listSources.GetItemCount(), source.as_posix())
    ...
    def insertSource(self, path):
        if path:
            self._view.listSources.InsertItem(
                self._view.listSources.GetItemCount(), path)
    ...
    def _updateModel(self):
        try:
            ...
            self._scheduleModel.setStartTime(*self._view.txtStartTime.GetTime())
            self._scheduleModel.setEndTime(*self._view.txtEndTime.GetTime())
